...
extern	uint8_t	mbx_hmi_to_seq[sizeof(uint32_t)];
...
uint32_t	i=0;
	while(DWIN_menus[i].char_id )
	{
		...
		{
			mbx_hmi_to_seq[0] = CMDPARSER_RET_LOAD;
			mbx_hmi_to_seq[1] = loaded_program = DWIN_menus[i].ee_program;
			mbx_send(PRESSO_SEQUENCER_PROCESS,PRESSO_HMI_MBX,mbx_hmi_to_seq,2);
			return 0;
		}
		i++;
	}
	...
	case	DWIN_PROG_PLAY:
		if ( loaded_program )
		{
			mbx_hmi_to_seq[0] = CMDPARSER_RET_EXEC;
			mbx_hmi_to_seq[1] = loaded_program;
			mbx_send(PRESSO_SEQUENCER_PROCESS,PRESSO_HMI_MBX,mbx_hmi_to_seq,2);
	...
	case	DWIN_PROG_STOP:
		if ( loaded_program )
		{
			mbx_hmi_to_seq[0] = CMDPARSER_RET_HLT;
			mbx_hmi_to_seq[1] = loaded_program;
			mbx_send(PRESSO_SEQUENCER_PROCESS,PRESSO_HMI_MBX,mbx_hmi_to_seq,2);
